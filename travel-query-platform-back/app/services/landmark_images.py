...
_wiki_fail_count = 0
_WIKI_CIRCUIT_BREAK_THRESHOLD = 2
...
async def resolve_landmark_images(
    ...
    *,
    ...
    global _wiki_fail_count
    if _wiki_fail_count < _WIKI_CIRCUIT_BREAK_THRESHOLD:
        try:
        ...
        for title in search_titles[:3]:
            try:
                ...
                if any("\u4e00" <= c <= "\u9fff" for c in title):
                    web_q = f"{title} 风景 地标"
                for u in await search_web_images_multi(web_q, max_results=max_images):
                    add(u)
                    if len(urls) >= max_images:
                        break
            except Exception as exc:
